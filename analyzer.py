...
class Analyzer:
    ...
    def __init__(self, llm_interaction_instance: Any, sentiment_model_id: Optional[str] = None, **kwargs):
        ...
        self.keyword_lists = {
            ...
            "gnostic_inquiry": ["гнозис", "деміург", "архонти", "софія", "втеча", "звільнення", "іскра", "в'язниця реальності", "матеріальний світ", "пробудження"],
            "spiritual_rejection": ["відмова", "зламати кайдани", "я не раб системи", "відкидаю матеріальне"]
        }
    ...
    def _infer_user_intent(self, analysis_results: Dict[str, Any]) -> str:
        ...
        # НОВІ НАМІРИ ДЛЯ ГНОСТИЦИЗМУ
        if intensities.get("gnostic_inquiry", 0) > 0:
            return "seek_gnosis"
        if intensities.get("spiritual_rejection", 0) > 0:
            return "perform_spiritual_rejection"
    ...
    def _analyze_psychological_state(self, analysis_results: Dict[str, Any]) -> str:
        ...
        # НОВІ СТАНИ ДЛЯ ГНОСТИЦИЗМУ
        if user_intent == "seek_gnosis":
            return "seeking_liberation"
        if user_intent == "perform_spiritual_rejection":
            return "awakening_rebel"
